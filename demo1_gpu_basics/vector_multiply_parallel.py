...
def mutiply(a, b, c, d):
     return  (a * (b ** 100) ** c * (d ** 240) * ((a + b) ** 9) * ((c + d) ** 100) ** ((a + d) ** 10))**1000
...
def main(vector_length):
    ...
    A = np.ones(vector_length).astype(np.float32) * 3
    B = np.ones(vector_length).astype(np.float32) * 4
    C = np.ones(vector_length).astype(np.float32) * 4
    D = np.ones(vector_length).astype(np.float32) * 5
    ...
    # Print results
    print("Result:   (a * (b ** 100) ** c * (d ** 240) * ((a + b) ** 9) * ((c + d) ** 100) * ((a + d) ** 10))**100")
    print(result)
    print("-----Stats----")
